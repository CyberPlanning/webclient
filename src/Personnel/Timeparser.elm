module Personnel.Timeparser exposing (toTime, decoder, toUtcString)
...
toUtcString time =
  ...
  ++ " " ++
  String.fromInt (monthToInt (Time.toMonth utc time))
  ++ " " ++
  String.fromInt (Time.toYear utc time)
  ++ " " ++
...
fromParts : Int -> Int -> Int -> Int -> Int -> Time.Posix
fromParts monthYearDayMs hour minute second utcOffsetMinutes =
    Time.millisToPosix
        (monthYearDayMs
            + (hour * 60 * 60 * 1000)
...
{-|  YYYYMMDDTHHmmssZ -> 20191112T123000Z
-}
            ...
            (\datePart ->
                oneOf
                    [ succeed (fromParts datePart)
                        ...
                        |= utcOffsetInMinutes
                        |. end
                    , succeed (fromParts datePart 0 0 0 0)
                        |. end
                    ]
            )
