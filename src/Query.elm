...
eventsApiQuery =
    "query day_planning($collection:String!,$grs:[String],$to:DateTime!,$from:DateTime!){planning(collection:$collection,affiliationGroups:$grs,toDate:$to,fromDate:$from){events{title startDate endDate classrooms teachers groups eventId}},hack2g2: planning(collection:\"planning_hack2g2\",toDate:$to,fromDate:$from){events{title,startDate,endDate,classrooms,teachers,groups,eventId}}}"
